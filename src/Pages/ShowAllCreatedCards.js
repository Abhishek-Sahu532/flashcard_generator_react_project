      ...
      <section className="flex flex-col text-slate-700">
        <header className="flex">
          ...
          <div className="flex flex-col">
            <h2 className={`text-xl text-${theme==='dark'?'white':'bg-slate-600'} font-bold`}>
              {ourCard.groupname}
            </h2>
            {ourCard.groupdescription && (
              <p className={` my-2 text-${theme==='dark'?'white':'bg-slate-600'} `}>{ourCard.groupdescription}</p>
            )}
          </div>
        </header>

        <main className="grid grid-rows-1 md:grid-cols-4 mt-6 mb-10">
          <aside className={`col-span-1 bg-${theme==='dark'?'dark':'white'}  w-[60vw] md:w-[10rem] xl:w-[17rem] mr-5 px-1 py-2 h-fit rounded-md border-2`}>
            ...
            {ourCard.cards &&
              ourCard.cards.map((card) => (
                <p
                  key={card.cardid}
                  className={`py-2 px-8 text-slate-700 bg-${theme==='dark'?'slate-600':'white'}  font-medium hover:bg-slate-100 cursor-pointer ${
...
<button type="button" className="absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next>
 <BiRightArrow />
          ...
          <section
            ref={componentRef}
            className={`col-span-3 md:col-span-2 flex flex-col xl:flex-row items-center w-full bg-${theme==='dark'?'dark':'white'}  shadow-lg rounded-lg md:flex-col border-2`}
          >
            <img
              src={demoImgCard}
              alt="cardimage"
            ...
            />
            <p className={`w-full p-6 py-10 text-${theme==='dark'?'white':'slate-600'}`} >{displayCard.carddescription} </p>
          </section>
          {/*right part */}
          <aside className="col-span-1 hidden md:flex flex-col items-center space-y-5">
            ...
            <div className="flex items-center py-3 px-4 xl:w-60 space-x-5  rounded-md shadow-lg active:scale-100 transition-all duration-100 hover:scale-105">
              <button className={`flex items-center py-3 px-4 xl:w-60 space-x-5 bg-${theme==='dark'?'dark':'white'} text-${theme==='dark'?'white':'slate-600'} rounded-md shadow-lg active:scale-100  transition-all duration-100 hover:scale-105 border-2`}>
                <IoDownloadOutline />
            ...
            <div className="flex items-center py-3 px-4 xl:w-60 space-x-5  rounded-md shadow-lg active:scale-100 transition-all duration-100 hover:scale-105">
              <ReactToPrint
                trigger={() => (
                  <button
                    onClick={handlePrint}
                    className={`flex items-center py-3 px-4 xl:w-60 space-x-5 bg-${theme==='dark'?'dark':'white'}
                    text-${theme==='dark'?'white':'slate-600'} rounded-md shadow-lg active:scale-100 transition-all duration-100 hover:scale-105 border-2`}
                  >
                    <IoPrintOutline />

...
const DefaultMyFlashcard = ({flashcard,theme}) => {
  ...
  return (
      <div
        ...
        className={`p-4 m-5 mx-auto flex flex-col space-y-3 items-center justify-center bg-${theme === "dark"?'dark':'white'}rounded-md text-black w-[23rem] h-[13rem] relative border-2 border-slate-200 word-break: break-all`}
      >
      ...
      <div className="absolute -top-9">
        ...
        <h2 className={`font-bold text-${theme === "dark"?'white':"black"}`}>{flashcard.groupname}</h2>
        <p className={`text-center font-medium text-sm text-${theme === "dark"?'white':"black"} line-clammp-2 overflow-hidden`}>

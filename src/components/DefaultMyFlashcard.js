...
const DefaultMyFlashcard = ({flashcard,theme}) => {
  ...
  return (
      ...
      <div
        ...
        className={`p-4 m-5 mx-auto flex flex-col space-y-3 items-center justify-center bg-${theme==='dark'?'dark':"white"} rounded-md text-black w-[23rem] h-[13rem] relative border-2 border-slate-200`}
      >
      ...
      <div className="absolute -top-9">
        ...
        <h2 className={`font-bold text-${theme==='dark'?'white':'slate-800'}`}>{flashcard.groupname}</h2>
        <p className={`text-center font-medium text-sm text-${theme==='dark'?'white':'salte-600'} line-clammp-2`}>
        ...
        <button
          ...
          className={`py-1 px-16 text-red-600 font-bold rounded-sm border-red-600 ring-2 ring-red-600 bg-${theme==='dark'?'white':'white'}`}
        >
          {" "}
          View Cards
        </button>
